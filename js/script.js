...
function typewriterNextOperation(container) {
  const { currentStringData, strings } = trypers[container];
  writeMode = strings[currentStringData.index].typeStyle;
  typewriterRemoveBlinkCurser(trypers[container].DOM_curser);
  switch (writeMode) {
    case "blink":
...
function typewriterBlink(container) {
  const { currentStringData, strings, DOM_typer } = trypers[container];
  const currentString = strings[currentStringData.index].string;
...
function typewriterDeleteTo(container) {
  ...
  console.log(
    `element: ${DOM_typer.innerHTML} string: ${currentString.slice(
      0,
      DOM_typer.innerHTML.length
    )} result: ${
  ...
  console.log(`string data: ${currentString}`);
  return typewriterCleanupPause(
    container,
    strings,
    strings[currentStringData.index].pauseFor
...
function typewriterAddBlinkCurser(container) {
  const { DOM_curser } = trypers[container];

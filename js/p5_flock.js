let folk_handeler;
const functions = {
  attractAlly: (diffrence,distence2)=>{return {x:(diffrence.x*distence2)/20000000000,y:(diffrence.y*distence2)/20000000000}},
  attractCenter: (diffrence,distence2)=>{return {x:(diffrence.x*distence2)/7000000,y:(diffrence.y*distence2)/7000000}},
  repelAlly: (diffrence,distence2)=>{return {x:-(diffrence.x*10)/(distence2/20000)/10000,y:-(diffrence.y*10)/(distence2/20000)/10000}},
  reactThreat:(diffrence,distence2)=>{return {x:-(diffrence.x*100)/(distence2/200000)/10000,y:-(diffrence.y*100)/(distence2/200000)/10000}}
}

function setup() {
  let canvasDiv = select('#header-canvas');
...
class Main{
  ...
  updateParticles(){
    for(let i = 0; frameCount%this.update_jump+i < this.particles.length; i+= this.update_jump){
      ...
      if(obj.pos.x != mouseX && obj.pos.y != mouseY)obj.allyReact({x:mouseX,y:mouseY});
      obj.update();
    }
  ...
  draw(){
    if(mouseIsPressed) this.particles.push(new Particle({x:mouseX,y:mouseY}));
    this.drawParticles();
    this.drawThreats();
  }
...
class Particle{
  constructor(pos_={"x":random(0,width),"y":random(0,width)},vel_={"x":0,"y":0}){
  ...
  threatReact(ThreatPos){
    const diffrence = {x:ThreatPos.x-this.pos.x, y:ThreatPos.y-this.pos.y};
    const distence2 = pow(diffrence.x,2)+pow(diffrence.y,2);
  ...
  drag(){
    this.vel.x = this.vel.x*(map(folk_handeler.particles.length, 50, 500, 0.99 ,0.9));
    this.vel.y = this.vel.y*(map(folk_handeler.particles.length, 50, 500, 0.99 ,0.9));
  }
...
class Threat{
  ...
  constrain(){
    ...
    this.vel.x = constrain(this.vel.x,-100,100);
    this.vel.y = constrain(this.vel.y,-100,100);
  }
